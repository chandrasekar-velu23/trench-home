...
interface VariationClaudeProps {
  hideLeft?: boolean;
  activeStepOverride?: number;
}

const VariationClaude: React.FC<VariationClaudeProps> = ({ hideLeft = false, activeStepOverride }) => {
  ...
  // Mutate global CT properties
  CT.bg = isDark ? '#0A0A0C' : '#F9F6F0';
  CT.panel = isDark ? '#1E1E24' : '#FFFFFF';
  CT.text = isDark ? '#F1EBDE' : '#1A1A1A';
  CT.textDim = isDark ? '#94A3B8' : '#5A5A5A';
  CT.textMute = isDark ? '#64748B' : '#8C8C8C';
  CT.line = isDark ? 'rgba(255,255,255,0.08)' : '#E8E4DC';
  CT.accent = '#D96B43'; // warm orange
  
  // Claude specific
  CT.claudeBg = isDark ? '#191919' : '#FAF8F5';
  CT.claudePanel = isDark ? '#1A1A1A' : '#FFFFFF';
  CT.claudeLine = isDark ? '#2D2D2D' : '#E3E0D8';
  CT.claudeText = isDark ? '#E3E3E3' : '#242424';
  CT.claudeDim = isDark ? '#B4B4B4' : '#66635C';
  ...
  const r7_visible = r7;
  const r8typing_visible = r8typing * (1 - r8);
  const r8_visible = r8;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* RIGHT — Claude window */}
        <div style={{
          flex: hideLeft ? 'none' : (isClient && window.innerWidth > 768 ? '0 0 65%' : '1'),
          ...
          <div ref={scrollRef} style={{
            ...
            {r4_visible > 0 && (
              ...
              >
                <div>The attempts originated from <strong>3 geographic clusters</strong>. Two are flagged critical: impossible travel detected for <strong>maya.chen</strong> (Berlin &rarr; Lagos in under 4 minutes).</div>
                <GeoList locations={[
