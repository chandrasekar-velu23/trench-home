...
interface VariationTeamsProps {
  hideLeft?: boolean;
  activeStepOverride?: number;
}

const VariationTeams: React.FC<VariationTeamsProps> = ({ hideLeft = false, activeStepOverride }) => {
  ...
  // Mutate global TT properties
  TT.bg = isDark ? '#0A0A0C' : '#ffffff';
  TT.bgDeep = isDark ? '#111217' : '#f8f9fa';
  TT.text = isDark ? '#F1EBDE' : '#213547';
  TT.textDim = isDark ? '#94A3B8' : '#6b7280';
  TT.textMute = isDark ? '#64748B' : '#9ca3af';
  TT.line = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)';
  TT.lineHi = isDark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.14)';
  
  // Teams specific
  TT.teamsBg = isDark ? '#1F1F1F' : '#ffffff';
  TT.teamsPanel = isDark ? '#141414' : '#f5f5f5';
  TT.teamsCard = isDark ? '#292929' : '#ffffff';
  TT.teamsLine = isDark ? '#3D3D3D' : '#e1e1e1';
  TT.teamsText = isDark ? '#E0E0E0' : '#242424';
  TT.teamsDim = isDark ? '#ADADAD' : '#616161';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            width: hideLeft || (isClient && window.innerWidth > 768) ? 64 : 0,
            display: hideLeft || (isClient && window.innerWidth > 768) ? 'flex' : 'none',
            background: '#f0f0f0',
