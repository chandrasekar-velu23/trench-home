...
interface VariationSlackProps {
  hideLeft?: boolean;
  activeStepOverride?: number;
}

const VariationSlack: React.FC<VariationSlackProps> = ({ hideLeft = false, activeStepOverride }) => {
  ...
  // Mutate global TD token colors so external components receive updated values
  TD.bg = isDark ? '#0A0A0C' : '#ffffff';
  TD.bgDeep = isDark ? '#111217' : '#f8f9fa';
  TD.panel = isDark ? '#1e1f24' : '#ffffff';
  TD.panelHi = isDark ? '#2a2b30' : '#f8f9fa';
  TD.line = isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.08)';
  TD.lineHi = isDark ? 'rgba(255,255,255,0.14)' : 'rgba(0,0,0,0.14)';
  TD.text = isDark ? '#F1EBDE' : '#213547';
  TD.textDim = isDark ? '#94A3B8' : '#6b7280';
  TD.textMute = isDark ? '#64748B' : '#9ca3af';
  TD.accent = isDark ? '#3152B9' : '#532455';
  TD.slackBg = isDark ? '#1A1D21' : '#ffffff';
  TD.slackPanel = isDark ? '#19171D' : '#f8f9fa';
  TD.slackLine = isDark ? '#2D3136' : '#e5e7eb';
  TD.slackText = isDark ? '#D1D2D3' : '#374151';
  TD.slackDim = isDark ? '#9A9B9C' : '#6b7280';
  TD.slackBlue = isDark ? '#36C5F0' : '#532455';
  TD.slackGreen = '#2BAC76';
  ...
  useEffect(() => {
    if (activeStepOverride !== undefined) return; // disable timer if step is overridden
    let frame: number;
  ...
  const autoplayPhase = (timeMs % cycleMs) / cycleMs;
  const phase = activeStepOverride !== undefined
    ? (activeStepOverride === 1 ? 0.15
       : activeStepOverride === 2 ? 0.42
       : activeStepOverride === 3 ? 0.60
       : 0.88)
    : autoplayPhase;
  ...
  return (
    <div style={{
      position: 'relative',
      width: '100%',
      minHeight: hideLeft ? '480px' : '400px',
      background: 'transparent',
      overflow: 'hidden',
      border: 'none',
      fontFamily: 'var(--font-secondary)',
      padding: hideLeft ? '0' : (isClient && window.innerWidth > 768 ? '24px 48px' : '16px 32px'),
      ...
      <div style={{
        display: 'flex',
        flexDirection: hideLeft ? 'column' : (isClient && window.innerWidth > 768 ? 'row' : 'column'),
        alignItems: hideLeft ? 'center' : 'stretch',
        gap: '20px',
        height: '100%',
      }}>
        {/* LEFT — copy + step list */}
        ...
        {/* RIGHT — Slack window */}
        <div style={{
          flex: hideLeft ? 'none' : (isClient && window.innerWidth > 768 ? '0 0 60%' : '1'),
          width: hideLeft ? '100%' : 'auto',
          maxWidth: hideLeft ? '840px' : 'none',
          minWidth: hideLeft ? 'unset' : (isClient && window.innerWidth > 768 ? 400 : 0),
          ...
          <div style={{
            ...
            <div style={{ padding: '14px 14px 4px', fontSize: 11, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Apps</div>
            <div style={{ padding: '4px 14px', display: 'flex', alignItems: 'center', gap: 8, color: '#ffffff' }}>
              <span style={{
