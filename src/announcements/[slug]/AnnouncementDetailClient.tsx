...
export default function AnnouncementDetailClient({ item }: Props) {
  // Sync canonical link and page title
  React.useEffect(() => {
    if (typeof window === "undefined") return;

    document.title = `${item.title} | Trench Security`;
    ...
    canonicalTag.setAttribute("href", `https://www.trenchsecurity.ai/announcements/${item.slug}`);
  ...
  return (
    <main className="announcements-page overflow-hidden">
      ...
      <div className="announcements-container" style={{ paddingBottom: 0 }}>
        <motion.div
          initial={isInitialMount() ? false : { opacity: 0, x: -12 }}
