...
export default function TrenchLabsPage() {
  ...
  useEffect(() => {
    ...
    const loadPdf = () => {
      ...
      if (!pdfjs) return;
      // Range requests: fetch only the pages shown, not the whole 12 MB file up front.
      pdfjs.getDocument({ url: activeSignal.pdfUrl, disableAutoFetch: true, disableStream: true }).promise.then((doc: any) => {
        setPdfDoc(doc);
        ...
        <section className="pdf-carousel-section" style={{ marginBottom: "5rem" }}>
          ...
          <ScrollReveal direction="up" delay={0.2} className="w-full">
            <div className="pdf-viewer-card">
              {/* The whole frame renders immediately, the canvas box reserved at
                  the PDF's square page size (1200x1200) with the spinner as an
                  overlay, so nothing moves when the document loads (CLS). */}
              ...
              <div className="pdf-display-area">
                <div className="pdf-canvas-wrap" style={{ position: "relative", aspectRatio: "1 / 1" }}>
                  <canvas
                    ref={canvasRef}
                  ...
                  />
                  {loading && (
                    <div className="pdf-loading" role="status" style={{ position: "absolute", inset: 0 }}>
              ...
              <div className="pdf-actions">
                <button onClick={handleZoomOut} disabled={loading} className="pdf-action-btn secondary" aria-label="Zoom Out">
                  <ZoomOut size={16} />
                  <span>Zoom Out</span>
                </button>

                <button onClick={handleZoomIn} disabled={loading} className="pdf-action-btn secondary" aria-label="Zoom In">
                  <ZoomIn size={16} />
                  <span>Zoom In</span>
                </button>
              </div>
